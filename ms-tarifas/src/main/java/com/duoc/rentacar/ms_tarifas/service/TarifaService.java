...
public class TarifaService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(TarifaService.class);
    ...
    public Tarifa obtenerPorTipoAuto(String idTipo) {
        ...
        logger.info(
                "Buscando tarifa aplicada para el tipo de auto ID: {}",
                idTipo
        );
        ...
        try {
            ...
            return tarifaRepository
                    .findByIdTipoAutoRef(idTipoLong)
                    .orElse(null);
    ...
    public Tarifa actualizar(Long id, Tarifa tarifa) {
        ...
        Tarifa tarifaExistente =
                tarifaRepository.findById(id).orElse(null);

        if (tarifaExistente != null) {

            tarifaExistente.setDescripcion(
                    tarifa.getDescripcion()
            );

            tarifaExistente.setValorPorDia(
                    tarifa.getValorPorDia()
            );

            tarifaExistente.setIdTipoAutoRef(
                    tarifa.getIdTipoAutoRef()
            );
